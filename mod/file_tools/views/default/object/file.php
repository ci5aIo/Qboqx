...
if ($full_view && !elgg_in_context("gallery")) {
	...
	echo "</div>";
} //@EDIT 2018-01-15 - SAJ - Added brief view for qbox list
elseif ($presentation == 'qbox' || $presentation == 'qbox_experience'){
	$file_guid = $file->getGUID();
	$item_guid = elgg_extract('item_guid', $vars);
	$attached_files = elgg_get_entities_from_relationship(array(
		'type' => 'object',
	...
	if ($attached_files){foreach($attached_files as $attached_file){$attached_file_guids[]=$attached_file->getGUID();}}
	$file_icon = elgg_view_entity_icon($file, "tiny", array("img_class" => "file-tools-icon-tiny"));
	$file_icon_options = ["name" => "jot[documents][]", "value" => $file_guid, "default" => false];
	if (in_array($file_guid, $attached_file_guids) && $presentation == 'qbox'){$file_icon_options['checked']='checked';$file_icon_options['disabled']='disabled';}
	$file_icon_alt = elgg_view("input/checkbox", $file_icon_options);	
	$params = array(
		'entity' => $file,
	);
	$params = $params + $vars;
