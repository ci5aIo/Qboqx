...
register_error(elgg_echo('coverphototime:file:empty'));  //TM:  timlineavatar
forward(REFERER);
}
...
$file->delete();
 ...
 system_message(elgg_echo('coverphototime:upload:success'));  //TM:  If we are here then the photos are ok by now
forward(REFERER);
}
